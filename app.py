...
# --- 5. SIDEBAR & UI CONTROLS ---
   ...
   # --- AUTOMATIC PERSONA MATCHING ---
    ...
    with tab3:
        ...
        st.markdown(
            "Quantitative performance assessment dynamically tracking changes across models."
        )
        ...
        # If current model is SVD, compare against Content-Based
        if selected_model == baseline_model:
            baseline_model = "Content-Based Filtering"

        try:
            current_row = eval_metrics_df[
                eval_metrics_df["Algorithm"] == selected_model
            ].iloc[0]

            baseline_row = eval_metrics_df[
                eval_metrics_df["Algorithm"] == baseline_model
            ].iloc[0]
            ...
            prec_diff = (
                current_row["Precision@5"]
                - baseline_row["Precision@5"]
            ) * 100

            rec_diff = (
                current_row["Recall@5"]
                - baseline_row["Recall@5"]
            ) * 100
